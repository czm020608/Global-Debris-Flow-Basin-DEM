...
from debris_flow_dem.functions.channel_metrics import compute_metrics, parse_float_list
from debris_flow_dem.functions.prepare_google_points import prepare_points
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Maximum allowed distance for snapping the input start point. "
            "Start points farther than this from the extracted stream are not relocated automatically."
        ),
    )
    parser.add_argument(
        "--outlet-max-snap-m",
